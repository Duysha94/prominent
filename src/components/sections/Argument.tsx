  ...
  return (
    <section className="ak-accent-field relative overflow-hidden py-24 md:py-32">
      <div className="mx-auto max-w-[1440px] px-5 md:px-10">
        ...
        >
          Strategy that never leaves the deck is not strategy.
        </CutText>
        ...
        <ol className="mt-16 grid gap-px border border-[var(--rule-strong)] bg-[var(--rule-strong)] sm:grid-cols-2 lg:grid-cols-4">
          {MOVEMENTS.map((movement, i) => (
            <Reveal key={movement.id} delay={0.08 * i} as="li">
              <div className="h-full bg-[var(--color-brand-500)] p-6">
                <span className="font-mono text-[0.5rem] uppercase tracking-[0.2em] opacity-60">
                  Handover {movement.index}
                </span>
                <h3 className="mt-3 font-display text-2xl italic">{movement.title}</h3>
                <p className="mt-3 text-[0.8125rem] leading-relaxed opacity-80">{movement.handover}</p>
              </div>
            </Reveal>
          ))}
        </ol>
