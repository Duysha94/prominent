...
function ak_project_register() {
	$register = array(
		// AK OWNED.
		array( 'key' => 'lfd', 'title' => 'London Fashion Day', 'rel' => 'ak-owned', 'type' => 'platform', 'url' => 'https://londonfashionday.co.uk', 'order' => 1, 'code' => 'AK-LFD' ),
		array( 'key' => 'ofd', 'title' => 'Odessa Fashion Day', 'rel' => 'ak-owned', 'type' => 'platform', 'url' => 'https://ofd.org.ua', 'order' => 5, 'code' => 'AK-OFD' ),
		array( 'key' => 'coolbaba', 'title' => 'COOLBABA', 'rel' => 'ak-owned', 'type' => 'media-editorial', 'url' => 'https://coolbaba.in.ua', 'order' => 2, 'code' => 'AK-COOL' ),
		array( 'key' => 'prominent', 'title' => 'Prominent Magazine', 'rel' => 'ak-owned', 'type' => 'media-editorial', 'url' => 'https://prominentmagazine.co.uk', 'order' => 6, 'code' => 'AK-PROM' ),
		array( 'key' => 'fashion-frontier', 'title' => 'Fashion Frontier', 'rel' => 'ak-owned', 'type' => 'platform', 'url' => 'https://fashionfrontier.uk', 'order' => 9, 'code' => 'AK-FF' ),
		array( 'key' => 'utrend', 'title' => 'Utrend Store', 'rel' => 'ak-owned', 'type' => 'retail-ecommerce', 'url' => 'https://utrendstore.co.uk', 'order' => 7, 'code' => 'AK-UTR' ),
		array( 'key' => 'keka', 'title' => 'KEKA', 'rel' => 'ak-owned', 'type' => 'fashion-brand', 'url' => 'https://keka.design', 'order' => 3, 'code' => 'AK-KEKA' ),
		// CLIENT. No type: an address and a client relationship establish that
		// an engagement happened, not what it was.
		array( 'key' => 'wolax', 'title' => 'Wolax', 'rel' => 'client', 'type' => null, 'url' => 'https://wolax.co.uk', 'order' => 4, 'code' => 'AK-WLX' ),
		array( 'key' => 'lenie-boya', 'title' => 'Lenie Boya', 'rel' => 'client', 'type' => null, 'url' => 'https://lenieboya.com', 'order' => 8, 'code' => 'AK-LB' ),
		array( 'key' => 'smyn', 'title' => 'Show Me Your Nails', 'rel' => 'client', 'type' => null, 'url' => 'https://showmeyournails.com', 'order' => 10, 'code' => 'AK-SMYN' ),
	);
...
function ak_seed_projects() {
	...
	foreach ( ak_project_register() as $entry ) {
		...
		/*
		 * Owner-managed values, seeded through ak_seed_field(): each writes
		 * only while the field is empty AND the owner has never edited it.
		 * That is the whole contract — after the owner touches one of these,
		 * no later release writes to it again, including to "restore" it.
		 */
		ak_seed_field( $post_id, 'ak_code', $entry['code'] );
		ak_seed_field( $post_id, 'ak_url', esc_url_raw( $entry['url'] ) );
		ak_seed_field( $post_id, 'ak_owner', 'ak-owned' === $entry['rel'] ? 'AK Brand Development Studio' : $entry['title'] );
		// The website module starts enabled in AUTO. If the capture never
		// verifies the module simply does not render, so the project is
		// publishable from the moment it is created.
		ak_seed_field( $post_id, 'ak_wp_enabled', 1 );
		ak_seed_field( $post_id, 'ak_wp_mode', 'auto' );

		// Position: seeded when still unset and untouched, never re-imposed.
		$existing_order = (int) get_post_field( 'menu_order', $post_id );
		if ( ! $existing_order && ! empty( $entry['order'] ) && ! ak_owner_edited( $post_id, 'menu_order' ) ) {
			wp_update_post( array( 'ID' => $post_id, 'menu_order' => (int) $entry['order'] ) );
		}

		// Relationship is RELEASE-MANAGED for a canonical seeded record: it is
		// confirmed fact about how the project came to exist, and the site
		// cannot be reasoned about if it drifts. Enforced every deployment.
		 ...
		 */
		if ( $entry['type'] && ! get_post_meta( $post_id, '_ak_type_seeded', true ) && ! ak_owner_edited( $post_id, 'tax:ak_project_type' ) ) {
	...
	/*
	 * Every project ends a deployment holding a stored code.
	 *
	 * ak_project_code() can mint one on read as a last resort, but a write
	 * during an anonymous GET is not where project identity should be
	 * established. Doing it here means the render path only ever reads.
	 */
	foreach ( get_posts( array(
		...
		if ( '' === (string) get_post_meta( $id, 'ak_code', true ) ) {
			update_post_meta( $id, 'ak_code', ak_mint_project_code( get_the_title( $id ), $id ) );
			$report['healed'][] = get_the_title( $id ) . ' — project code minted';
		}
	}
	...
	foreach ( ak_project_fixtures() as $entry ) {
		...
		if ( ! $post_id ) {
			...
			ak_mark_post( $post_id, 'project_' . $entry['key'] );
			ak_seed_field( $post_id, 'ak_code', 'AK-FIX-' . strtoupper( substr( $entry['key'], 8, 3 ) ) );
			$type = get_term_by( 'slug', $entry['type'], 'ak_project_type' );
			if ( $type && ! is_wp_error( $type ) ) {
				wp_set_object_terms( $post_id, array( (int) $type->term_id ), 'ak_project_type', false );
			}
